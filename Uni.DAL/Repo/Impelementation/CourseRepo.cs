...
namespace Uni.DAL.Repo.Impelementation
{
    ...
    public class CourseRepo : ICourseRepo
    {
        ...
        public async Task<List<Course>> GetCoursesAsync(string department, string year, string semester) =>
            await _context.Courses
                .Where(c => c.DeptName == department && c.Year == year && c.Semester == semester)
                .ToListAsync();

        public async Task SaveTakesAsync(List<Takes> takes)
        {
            await _context.Takes.AddRangeAsync(takes);
            await _context.SaveChangesAsync();
        }

        public async Task<int> GetTotalCreditsAsync(List<string> courseCodes)
        {
